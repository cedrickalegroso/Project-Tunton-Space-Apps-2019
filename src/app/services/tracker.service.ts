...
export class TrackerService {
  ...
  async addDataMock() {
    ...
    let randomizerP = Math.floor(Math.random() * (3 - 1 + 1) + 1);

    let places = ['', 'SM MOA', 'Luzon Public Market', 'Tarlac Mall', 'Greenhills'];


    let timeinMock = Math.floor(Math.random() * (42400 - 32400 + 1) + 32400);
    let timeoutMock = Math.floor(Math.random() * (61200 - 42400 + 1) + 42400);

    function secondsToTime(timeinMock)
    {
       let secs = timeinMock
    ...
    function secondsToTime2(timeoutMock)
    {
      let secs1 = timeoutMock
        var hours = Math.floor(secs1 / (60 * 60));
    
        var divisor_for_minutes = secs1 % (60 * 60);
        var minutes = Math.floor(divisor_for_minutes / 60);
    ...
    const timeInHMS = await secondsToTime(timeinMock)
    const timeoutHMS = await secondsToTime2(timeoutMock)
   
    let finalTimeIn =  timeInHMS.h +  ":" + timeInHMS.m + ":" + timeInHMS.s
    let finalTimeOut =  timeoutHMS.h +  ":" + timeoutHMS.m + ":" + timeoutHMS.s
  ...
  //  console.log(timeinMock, timeoutMock, finalTimeIn, finalTimeOut, places[randomizerP] )



    this.afs.collection('mockDataActions').add({
      name: full,
      place: places[randomizerP],
      timein: timeinMock,
      timeout: timeoutMock,
      timein2: finalTimeIn,
      timeout2: finalTimeOut,
      controlNo: controlFinal.toString(),
      vulnerability: vulnerability,
      medicalCondition: [medicalConditionsList[randomizer]]
    });
